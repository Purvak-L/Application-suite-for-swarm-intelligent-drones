...
void position_calc(vector<AprilTags::TagDetection> detections, int ntags, 
  vector<Eigen::Vector3d> loc_tags, Eigen::Vector3d &abs_pos, Eigen::Vector3d &abs_rot);
/**
...
public:
  ...
  void position_calc(vector<AprilTags::TagDetection> detections, int ntags,vector<Eigen::Vector3d> loc_tags, Eigen::Vector3d &abs_pos, Eigen::Vector3d &abs_rot)
  {
    // Function calculates the position wrt multiple tags
    ...
    for (int i = 0; i <detections.size(); i++)
    {
      ...
      // x,y,z values
      abs_pos += loc_tags[detections[i].id] + translation;
      // yaw pitch roll
      abs_rot +=  Eigen::Vector3d(yaw, pitch, roll);
    }

    abs_pos /= detections.size();
    abs_rot /= detections.size();
  ...
  void processImage(cv::Mat& image, cv::Mat& image_gray) {
    ...
    // testing values
    vector<Eigen::Vector3d> loc_tags(3);
    loc_tags[0] = Eigen::Vector3d(0,0,0);
    loc_tags[1] = Eigen::Vector3d(0,0.13,0);
    loc_tags[2] = Eigen::Vector3d(0,0,0.08);
    Eigen::Vector3d abs_pos(0.0,0.0,0.0);
    Eigen::Vector3d abs_rot(0.0,0.0,0.0);
    position_calc(detections,3,loc_tags,abs_pos,abs_rot);
